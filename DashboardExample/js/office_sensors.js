$(document).ready(function() {
    ...
    addGraphToolTip("overview", $("#office-overview-graph"), ["°C", "°C", "°C"]);

    // Set up Image Map & Settings
    $('#ei-floorplan').mapster({
        render_highlight: { fillOpacity: 0.1, },
        render_select: { fillOpacity: 0.3, },
        fillColor: "95AD93",
        stroke: true,
        strokeColor: "206118",
        strokeOpacity: 0.8,
    ...
    function findActiveSidebar() {
        return $(".nav-sidebar .active").attr("data-room-name");
    }
    ...
    function showCurrentTab(tab) {
        ...
        // remove current tab from array of navTabs
        ...
        // add current tab back to array of navTabs
        navTabs.push(tab);
    }
    ...
    function createRow(deviceInfo) {
        $(".device-rows").append("<div class=row data-room-id="+deviceInfo.agentID+"></div>");
    }

    function createDeviceWidget(deviceInfo, temp, humid, light) {
        $(".device-rows [data-room-id="+deviceInfo.agentID+"]").prepend('<div class="col-sm-7"><div class="room-widgets" data-room-id="'+deviceInfo.agentID+'"><h3>'+deviceInfo.name+'</h3><p class="title-border"></p>'+temp+humid+light+'</div></div>' )
    }
    ...
    function createTempHumidGraph(heading, agentID) {
        ...
        $(".row[data-room-id="+agentID+"]").append('<div class="col-xs-6 col-sm-5 right"><div class="temp-humid-graph"><h4>'+heading+'</h4><p class="title-border"></p><div class="sm-graph" id="'+agentID+'-temp-humid-graph"></div></div></div>');
    }
    ...
    function addGraphToolTip(id, graph, toolTipText) {
        //look into automating temp default
    ...
    // this gets called for each active device
...
///////////////// weather API & Helpers ///////////////////
    ...
    function collectWeatherAPIData(parsed_json, weatherData) {
        ...
        for(var i = 0; i < history.length; i++) {
            var d = history[i]['utcdate'];
            var ts = Date.UTC( d.year, d.mon - 1, d.mday, d.hour, d.min ); //js Date month is indexed from 0 not 1 (ie jan = 0)
            ...
            if ( ts > areaStartTime && ts < now ) {
                weatherData.areaData.push([ ts, temp ]);
            }
        }
        return weatherData;
    }
